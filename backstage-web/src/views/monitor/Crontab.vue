<template>
  <el-form inline>
    <el-form-item label="搜索">
      <el-input v-model="crontab.query.name" placeholder="名称" clearable></el-input>
    </el-form-item>
    <el-form-item>
      <el-select v-model="crontab.query.status" placeholder="状态" clearable>
        <el-option label="启动" value=true></el-option>
        <el-option label="停止" value=false></el-option>
      </el-select>
    </el-form-item>
    <el-form-item>
      <el-button type="primary" @click="queryCrontab">查询</el-button>
    </el-form-item>
    <el-form-item>
  ...
  <el-table
    ...
    <el-table-column label="操作" min-width="360" fixed="right">
      <template #default="scope">
        ...
        <el-button
            type="danger"
            @click="deleteCrontab(scope.row)">删除
        </el-button>
      </template>
    </el-table-column>
  </el-table>
  ...
  <div style="display: flex; justify-content: center;">
    <el-pagination
        ...
        :total="crontab.total"
        :current-page="crontab.query.pageNo"
        @current-change="pageCrontab">
    </el-pagination>
  </div>
  ...
  <el-dialog title="修改任务" width="30%" center v-model="show.edit">
    ...
    <template #footer>
      <el-button type="primary" @click="editCrontab()">修改</el-button>
...
export default {
  ...
  methods: {
    loadCrontab() {
      ...
      this.$axios.get(crontabListUrl, {
        params: this.crontab.query,
      }).then(resp => {
        this.tableLoading = false;
        if (!resp.status) {
          return;
        }
        this.crontab.data = resp.data.list;
        this.crontab.total = resp.data.total;
      })
    },
    pageCrontab(page) {
      this.crontab.query.pageNo = page;
      this.loadCrontab();
    },
    queryCrontab() {
      this.crontab.query.pageNo = 1;
      this.loadCrontab();
    },
    openAddForm() {
      this.show.add = true;
    ...
    addCrontab() {
      this.$refs['addForm'].validate(valid => {
        ...
        this.$axios.post(crontabAddUrl, this.body.add)
            .then(resp => {
              if (!resp.status) {
                return;
              }
    ...
    editCrontab() {
      this.$refs['editForm'].validate(valid => {
        ...
        this.$axios.put(crontabEditUrl, this.body.edit)
            .then(resp => {
              if (!resp.status) {
                return;
              }
    ...
    editStatus(crontab) {
      ...
      this.$confirm(msg, '提示', {
        ...
        let body = {};
        body.id = crontab.id;
        body.status = !crontab.status;
        this.$axios.put(crontabEditStatusUrl, body)
            .then(resp => {
              if (!resp.status) {
                return;
              }
    ...
    deleteCrontab(data) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        this.$axios.delete(crontabDeleteUrl + data.id)
            .then(resp => {
              if (!resp.status) {
                return;
              }
    ...
    start(row) {
      this.$axios.post(crontabStartUrl + row.id)
          .then(resp => {
            if (!resp.status) {
              return;
            }
    ...
    openLog(data) {
      this.log.query.crontabId = data.id;
    ...
    loadLog() {
      ...
      this.$axios.get(crontabLogListUrl, {
        ...
        this.log.data = resp.data.list;
        this.log.total = resp.data.total;
      });
    },
    pageLog(page) {
