package com.tuccicode.backstage.app.handler;
...
public class GlobalExceptionHandler {
    ...
    public Response unauthorizedExceptionHandler(UnauthorizedException e) {
        return Response.fail(BackstageBizCode.UNAUTHORIZED.getCode(), BackstageBizCode.UNAUTHORIZED.getMessage());
    }
    ...
    public Response constraintViolationException(ConstraintViolationException e) {
        ...
        return Response.fail(BackstageBizCode.PARAMETER_ERROR.getCode(), msg);
    }
    ...
    public Response throwable(Throwable e) {
        BackstageBizCode bizCode;
        if (e instanceof HttpRequestMethodNotSupportedException) {
            // 请求方法不支持
            bizCode = BackstageBizCode.METHOD_NOT_ALLOWED;
        } else if (e instanceof HttpMessageNotReadableException) {
            // json解析错误
            bizCode = BackstageBizCode.JSON_PARSE_ERROR;
        } else if (e instanceof HttpMediaTypeNotSupportedException) {
            // 媒体类型不支持
            bizCode = BackstageBizCode.UNSUPPORTED_MEDIA_TYPE;
        } else if (e instanceof MethodArgumentTypeMismatchException) {
            // 参数类型转换错误
            bizCode = BackstageBizCode.PARAMETER_TYPE_ERROR;
        } else if (e instanceof MultipartException) {
            bizCode = BackstageBizCode.FILE_SIZE_LIMIT;
        } else {
            bizCode = BackstageBizCode.SERVER_ERROR;
            log.error("未知异常", e);
        }
