package com.tuccicode.backstage.domain.core.exception;

import com.tuccicode.concise.exception.BizCode;

/**
 * @author tucci.lee
 */
public enum BackstageBizCode implements BizCode {
    ...
    CRONTAB_NAME_EXIST(51001, "定时任务名称已经存在"),
    CRONTAB_ADD_ERROR(51002, "定时任务添加错误"),
    CRONTAB_DELETE_ERROR(51003, "定时任务删除错误"),
    CRONTAB_RESUME_ERROR(51004, "定时任务恢复错误"),
    CRONTAB_PAUSE_ERROR(51005, "定时任务暂停错误"),
    CRONTAB_START_ERROR(51006, "定时任务执行失败"),;

    BackstageBizCode(int code, String message) {
        this.code = code;
